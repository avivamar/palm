...
// packages/shopify/src/next/handler.ts
import { NextResponse } from "next/server";

// packages/shopify/src/config/index.ts
import { z } from "zod";
var ShopifyConfigSchema = z.object({
  // 基础配置
  storeDomain: z.string().min(1),
  adminAccessToken: z.string().startsWith("shpat_"),
  ...
  timeoutMs: z.number().int().min(1e3).default(1e4),
  batchSize: z.number().int().min(1).max(250).default(50),
  syncInterval: z.number().int().min(6e4).default(3e5),
  // 最小1分钟
  // Webhook 配置
  webhook: z.object({
...
// packages/shopify/src/core/error-handler.ts
...
// packages/shopify/src/services/products.ts
...
// packages/shopify/src/services/orders.ts
...
// packages/shopify/src/services/inventory.ts
...
// packages/shopify/src/services/customers.ts
...
// packages/shopify/src/monitoring/health-check.ts
...
// packages/shopify/src/monitoring/metrics.ts
var Metrics = class {
  constructor(config) {
    this.metricsData = {
      apiCalls: 0,
...
// packages/shopify/src/core/integration.ts
...
var WebhookHandler = class {
  ...
  verifyWebhookSignature(rawBody, signature) {
    ...
    const hash = crypto.createHmac("sha256", this.config.webhook.secret).update(rawBody, "utf8").digest("base64");
...
function createShopifyApiRoute(config) {
