...
export const AnimatedBeam = forwardRef(({ className, containerRef, fromRef, toRef, curvature = 0, reverse = false, duration = Math.random() * 3 + 4, delay, pathColor = 'gray', pathWidth = 2, pathOpacity = 0.2, gradientStartColor = '#ffaa40', gradientStopColor = '#9c40ff', startXOffset = 0, startYOffset = 0, endXOffset = 0, endYOffset = 0, }, _ref) => {
    const id = useId();
    ...
    useEffect(() => {
        ...
        const updatePath = () => {
            ...
            (_a = pathRef.current) === null || _a === void 0 ? void 0 : _a.setAttribute('d', d);
        };
    ...
    return (<svg ref={svgRef} width="100%" height="100%" viewBox="0 0 100 100" style={{
            position: 'absolute',
            pointerEvents: 'none',
            top: 0,
            left: 0,
        ...
        <path ref={pathRef} fill="none" stroke={pathColor} strokeWidth={pathWidth} strokeOpacity={pathOpacity} strokeLinecap="round"/>
        <path fill="none" stroke={`url(#${id}-gradient${reverse ? '-reverse' : ''})`} strokeWidth={pathWidth} strokeLinecap="round" strokeDasharray="20 20" className={cn('animate-beam', reverse && 'animate-beam-reverse')} style={{
...
//# sourceMappingURL=animated-beam.jsx.map